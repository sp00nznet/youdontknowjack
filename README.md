...
| **Render-loop gate** | ⏳ **deeper drift** | `func_003DC468` still spins in a *second* loop whose limit is another callee-saved reg, corrupted by an **unprotected tail-entry** (same class, one level deeper). Closing the remaining ~452 merged functions removes these. See [`docs/BOOT_TRACE.txt`](docs/BOOT_TRACE.txt) handoff. |
