...
| **Render-loop gate** | ⏳ **frame-drift** | one guest thread wedges in a runaway loop: a count register reads the **TOC pointer** instead of a small value, because a callee deep in the call tree drifts its stack pointer and clobbers a saved register — the same bug class as the boundary fix, one layer deeper. Pinpointed; the lifter's mid-function tail-entry pass needs to honor the recovered boundaries. See [`docs/BOOT_TRACE.txt`](docs/BOOT_TRACE.txt) handoff. |
